...
def load_prices(closing_prices,closing_dates):
    ...
    with open(path+"ParseArticles/daily_AAPL.csv", 'rb') as f:
        lines = f.readlines()
...
def label_data_all_dates(labels,closing_prices,closing_dates):
    PERCENT=0.0
    for j in range(4288,len(closing_dates)-10):
...
def load_data():
    ...
    train_data=data[:int(NUM_SAMPLES*.8)]
    train_labels=labels[:int(NUM_SAMPLES*.8)]
    train_data,train_labels = shuffle(train_data,train_labels, random_state=0)
    
    test_data=data[int(NUM_SAMPLES*.8):]
    test_labels=labels[int(NUM_SAMPLES*.8):]
    ...
    return [shared(training_data), shared(validation_data), shared(testing_data), article_timestamps[int(NUM_SAMPLES*.8):], closing_prices, closing_dates]
